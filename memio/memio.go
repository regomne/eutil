// Package memio is an expandable ReadWriteSeeker for Golang that works with an
...
// NewWithBytes create a byte stream with a given bytes initialized
func NewWithBytes(stm []byte) *MemoryStream {
	buff := make([]byte, len(stm))
	copy(buff, stm)
	return &MemoryStream{buff: buff, loc: 0}
}
...
// AddCapacity adds the max capacity of the internal buffer
func (m *MemoryStream) AddCapacity(size int) {
	newBuff := make([]byte, len(m.buff), cap(m.buff)+size)
	copy(newBuff, m.buff)
	m.buff = newBuff
}
